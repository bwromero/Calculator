...
function calculate() {
  ...
  switch (
    calculatorState.operation // we perform the operation
  ) {
    case "+":
      result = prev + current;
      break;
...
function updateStateKey(state, key, newValue) {
  if (state[key] !== newValue) {
    //if the new value has not been setted yet, we update it
    state[key] = newValue;
...
function deleteValue(state, key, isOperator = false) {
  return updateStateKey(
    state,
    key,
    isOperator ? "" : truncateNumber(state[key])
  );
}
...
function setCalculatorState({
  previousNumber = calculatorState.previousNumber,
...
isExpressionPresent = () => {
  // we check if an expression has been inserted, by checking if every key has a value
